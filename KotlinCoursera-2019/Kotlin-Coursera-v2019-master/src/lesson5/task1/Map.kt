...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for (man in keys union friends.keys) {
        val shakes = mutableSetOf<String>();
        searching(friends, man, shakes, man)
        answer[man] = shakes;
    }
    return answer;
}
    ...
    if (friends != null || friends != 0) {
        for (friend in friends) {
            if (friend !== root && !visited.contains(friend)) {
                shakes.add(friend)
                visited.add(friend)
                searching(people, friend, shakes, root, visited);
            }
        }
    }
}
